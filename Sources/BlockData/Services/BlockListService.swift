...
public final class BlockListService: ObservableObject {
    ...
    public func reloadCallDirectoryExtension() {
        #if os(iOS)
        let primaryID = callExtensionID
        print("🔄 Requesting CallDirectory reload for: \(primaryID)...")
        CXCallDirectoryManager.sharedInstance.reloadExtension(withIdentifier: primaryID) { error in
            if let error = error {
                print("ℹ️ Reload primary extension (\(primaryID)): \(error.localizedDescription)")
                let fallbackID = "com.tranthien.test.CallDirectoryExt"
                if fallbackID != primaryID {
                    CXCallDirectoryManager.sharedInstance.reloadExtension(withIdentifier: fallbackID) { fbError in
                        if let fbError = fbError {
                            print("ℹ️ Reload fallback extension (\(fallbackID)): \(fbError.localizedDescription)")
                        } else {
                            print("✅ CallDirectory extension (\(fallbackID)) successfully reloaded")
                        }
                    }
                }
            } else {
                print("✅ CallDirectory extension (\(primaryID)) successfully reloaded")
            }
        }
        #endif
    }
